...
class SoftmaxClassifier:
    def __init__(self, map, latenSize, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5,
        ...
        self.cuda = _cuda
        self.model = MLP_LOGSOFTMAX(self.latent_dim, self.nclass)
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            for i in range(0, self.ntrain, self.batch_size):
                ...
                mus,stds,dis_out = self.map(inputv)
    ...
    def val_gzsl(self, test_X, test_label, target_classes):
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                mus,stds,dis_out = self.map(test_X[start:end].cuda())
                output = self.model(mus)
            else:

                mus,stds,dis_out = self.map(test_X[start:end])
                output = self.model(mus)
...
class MLP_LOGSOFTMAX(nn.Module):
    def __init__(self, input_dim, nclass):
        super(MLP_LOGSOFTMAX, self).__init__()
        # self.fc = nn.Linear(input_dim, int(input_dim/2))
        # self.relu = nn.ReLU()
        # self.fc1 = nn.Linear(int(input_dim/2), nclass)
